...
export function getChainConfig(chainId: number) {
  const key = `netId${chainId}`;
  const deployment = CONFIG.deployments[key as keyof typeof CONFIG.deployments];
...
type TokenConfig = {
  instanceAddress: Record<string, Address>;
  deployedBlockNumber: Record<string, number>;
...
export function getTokenConfig(chainId: number, targetAddress: Address): TokenConfig {
  const chainConfig = getChainConfig(chainId);
  if (!chainConfig) {
    throw new Error(`Unsupported chain ID: ${chainId}`);
  }

  for (const [tokenSymbol, tokenConfig] of Object.entries(chainConfig)) {
    if ('tokenAddress' in tokenConfig && tokenConfig.tokenAddress.toLowerCase() === targetAddress.toLowerCase()) {
      return tokenConfig as TokenConfig;
    }
  }

  throw new Error(`Token with address ${targetAddress} not found on chain ID ${chainId}`);
}

export function getEthConfig(chainId: number): TokenConfig {
  const chainConfig = getChainConfig(chainId);
  if (!chainConfig) {
    throw new Error(`Unsupported chain ID: ${chainId}`);
  }

  const ethConfig = chainConfig['eth'];
  if (!ethConfig) {
    throw new Error(`ETH configuration not found on chain ID ${chainId}`);
  }

  return ethConfig as TokenConfig;
}
