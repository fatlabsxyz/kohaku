...
import { IPoolDepositEvent, IWithdrawalEvent, IRagequitEvent, IPool } from "../../data/interfaces/events.interface";
...
export interface SyncEventsThunkParams {
    myPoolsSelector: () => IPool[];
    dataService: IDataService;
}

export const syncEventsThunk = createAsyncThunk<bigint, SyncEventsThunkParams, { state: RootState }>(
    'sync/syncEvents',
    async ({ dataService, myPoolsSelector }, { getState, dispatch }) => {
        const state = getState();
        const myPools = myPoolsSelector();
        ...
        myPools.map(async (pool) => {
            const events = await dataService.getPoolEvents({
                ...
                fromBlock: lastSyncedBlock === 0n ? pool.registeredBlock : lastSyncedBlock + 1n,
        ...
        results.forEach((result) => {
            if (result.status === 'fulfilled') {
                const { PoolDeposited, Withdrawn, Ragequit, toBlock, pool } = result.value;
